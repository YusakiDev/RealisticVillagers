...
public class CustomBlockData implements PersistentDataContainer {
    ...
    static void setDirty(Plugin plugin, Map.Entry<UUID, BlockVector> blockEntry) {
        ...
        me.matsubara.realisticvillagers.RealisticVillagers rv = (me.matsubara.realisticvillagers.RealisticVillagers) plugin;
        rv.getFoliaLib().getScheduler().runNextTick(task -> DIRTY_BLOCKS.remove(blockEntry));
    }
    ...
    @Contract("_, _ -> new")
    private static @NotNull NamespacedKey getKey(Plugin plugin, Block block) {
     ...
     */
    @Nullable
    static Block getBlockFromKey(final @NotNull NamespacedKey key, final Chunk chunk) {
    ...
    public static boolean hasCustomBlockData(Block block, Plugin plugin) {
        return block != null && block.getChunk().getPersistentDataContainer().has(getKey(plugin, block), PersistentDataType.TAG_CONTAINER);
    }
     ...
     */
    public static @Nullable PersistentDataType<?, ?> getDataType(PersistentDataContainer pdc, NamespacedKey key) {
    ...
    @Override
    public @NotNull <T, Z> Z getOrDefault(final @NotNull NamespacedKey namespacedKey, final @NotNull PersistentDataType<T, Z> persistentDataType, final @NotNull Z z) {
    ...
    private static final class DataType {
        ...
        private static final PersistentDataType<Byte, Boolean> BOOLEAN = new PersistentDataType<>() {
