...
public class CustomGossipContainer extends GossipContainer {
    ...
    private Collection<GossipEntry> selectGossipsForTransfer(Random random, int topics) {
    ...
    public void transferFrom(@NotNull CustomGossipContainer container, Random random, int topics) {
    ...
    public void update(@NotNull Dynamic<?> dynamic) {
        dynamic.asStream().map(GossipEntry::load)
    ...
    private record GossipEntry(UUID target, GossipType type, int value) {
        ...
        public <T> @NotNull Dynamic<T> store(DynamicOps<T> ops) {
            return new Dynamic<>(ops, ops.createMap(ImmutableMap.of(
                    ...
                    SerializableUUID.CODEC.encodeStart(ops, target).result().orElseThrow(RuntimeException::new),
