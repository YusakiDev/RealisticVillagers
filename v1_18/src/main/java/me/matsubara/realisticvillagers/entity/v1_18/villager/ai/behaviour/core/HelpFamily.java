...
public class HelpFamily extends Behavior<Villager> implements Exchangeable {
    ...
    public void tick(ServerLevel level, Villager villager, long time) {
        ...
        ItemStack copy = food.isEmpty() ? ItemStack.EMPTY : food.copy();
        copy.setCount(1);

        npc.drop(copy, npc.getPlugin().getIgnoreItemKey());
    ...
    private boolean canHelpNearestHungry(Villager villager) {
        return getNearestHungry(villager).isPresent() && hasAnyMatching(villager.getInventory(), item -> Consume.isSafeFood(item.getItem()));
    }
    ...
    private boolean canHelpNearestPoisoned(Villager villager) {
        return getNearestPoisoned(villager).isPresent() && hasAnyMatching(villager.getInventory(), item -> item.is(Items.MILK_BUCKET));
    }
    ...
    private boolean canHelpNearestDamaged(Villager villager) {
        return getNearestDamaged(villager).isPresent() && hasAnyMatching(villager.getInventory(), item -> Consume.POTION_PREDICATE.test(item, Consume.HEALTH_EFFECTS));
    }
    ...
    private boolean hasAnyMatching(@NotNull Container container, Predicate<ItemStack> predicate) {
