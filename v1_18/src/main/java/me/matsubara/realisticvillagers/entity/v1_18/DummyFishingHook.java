...
public class DummyFishingHook extends FishingHook {
    ...
    public DummyFishingHook(VillagerNPC npc, Level level, int luck, int lureSpeed) {
        ...
        setDeltaMovement(delta.multiply(
                0.6d / delta.length() + 0.5d + random.nextGaussian() * 0.0045d,
                0.6d / delta.length() + 0.5d + random.nextGaussian() * 0.0045d,
                0.6d / delta.length() + 0.5d + random.nextGaussian() * 0.0045d));
    ...
    public void defineSynchedData() {
        ...
        getEntityData().define(BITING, false);
    }
    ...
    public void tick() {
        ...
        if (onGround) {
            ...
            // Retrieve if on ground for more than 5 seconds.
            if (life >= 100) {
    ...
    private void catchingFish(@NotNull BlockPos pos) {
        ...
        if (timeUntilHooked > 0) {
            timeUntilHooked -= i;
            if (timeUntilHooked > 0) {
                fishAngle += (float) (random.nextGaussian() * 4.0d);
                ...
                data = level.getBlockState(new BlockPos(x, y - 1.0d, z));
    ...
    public int retrieve(ItemStack item) {
        ...
        if (hookedIn != null) {
            ...
            LootTable table = level.getServer().getLootTables().get(BuiltInLootTables.FISHING);
            Iterator<ItemStack> iterator = table.getRandomItems(lootBuilder.create(LootContextParamSets.FISHING)).iterator();
