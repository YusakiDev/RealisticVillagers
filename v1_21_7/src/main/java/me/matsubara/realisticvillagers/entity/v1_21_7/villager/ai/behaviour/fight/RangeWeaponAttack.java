package me.matsubara.realisticvillagers.entity.v1_21_7.villager.ai.behaviour.fight;
    ...
    private void crossbowAttack(VillagerNPC npc, LivingEntity target) {
        if (state == CrossbowState.UNCHARGED) {
            ...
            if (isCrossbow) {
                ...
                // Check if weapon is still valid and is a ProjectileWeaponItem
                if (!weapon.isEmpty() && weapon.getItem() instanceof ProjectileWeaponItem projectileWeapon) {
                    ItemStack projectile = npc.getProjectile(projectileWeapon);
            ...
            // Check if weapon is still valid and is a ProjectileWeaponItem before getting projectile
            if (weapon.isEmpty() || !(weapon.getItem() instanceof ProjectileWeaponItem projectileWeapon)) {
                state = CrossbowState.UNCHARGED;
                return;
            }
            
            ItemStack arrow = npc.getProjectile(projectileWeapon);
