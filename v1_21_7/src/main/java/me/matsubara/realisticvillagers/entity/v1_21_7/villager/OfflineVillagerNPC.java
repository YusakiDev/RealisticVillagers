...
public class OfflineVillagerNPC implements IVillagerNPC {
    ...
    public static final BiFunction<VillagerTracker, Tag, IVillagerNPC> OFFLINE_MAPPER = (tracker, input) -> input instanceof CompoundTag compound ?
            ...
            tracker.getOffline(loadUUIDFromTag(input));
    ...
    public static @NotNull OfflineVillagerNPC from(CompoundTag tag) {
        ...
        return new OfflineVillagerNPC(getUUIDFromTag(tag, UUID), tag, position);
    }

    public CompoundTag getTag() {
        CompoundTag tag = this.tag.copy();
        putUUIDToTag(tag, UUID, uuid);
    ...
    public boolean isPartnerVillager() {
        return tag.getBoolean(IS_PARTNER_VILLAGER).orElse(false);
    }
    ...
    public boolean isFatherVillager() {
        return tag.getBoolean(IS_FATHER_VILLAGER).orElse(false);
    }
    ...
    private @Nullable IVillagerNPC getFamily(@NotNull CompoundTag tag, String who) {
        ...
        if (tag.contains(who)) { // Check if tag exists
            UUID partnerUUID = getUUIDFromTag(tag, who);
            return plugin.getTracker().getOffline(partnerUUID);
        } else {
            return tag.getCompound(who).map(OfflineVillagerNPC::from).orElse(null);
        }
    }
    ...
    public String getSex() {
        return tag.getString(SEX).orElse("");
    }
    ...
    public int getSkinTextureId() {
        return tag.getInt(SKIN_TEXTURE_ID).orElse(-1);
    }
    ...
    public int getKidSkinTextureId() {
        return tag.getInt(KID_SKIN_TEXTURE_ID).orElse(-1);
    }
    ...
    public long getLastProcreation() {
        return tag.getLong(LAST_PROCREATION).orElse(0L);
    }
    ...
    public int getFoodLevel() {
        return tag.getInt("foodLevel").orElse(20);
    }
    ...
    public boolean isEquipped() {
        return tag.getBoolean(EQUIPPED).orElse(false);
    }
    ...
    public Object getShoulderEntityLeft() {
        return tag.getCompound(OfflineVillagerNPC.SHOULDER_ENTITY_LEFT).orElse(new CompoundTag());
    }
    ...
    public Object getShoulderEntityRight() {
        return tag.getCompound(OfflineVillagerNPC.SHOULDER_ENTITY_RIGHT).orElse(new CompoundTag());
    }
    ...
    public boolean isWasInfected() {
        return tag.getBoolean(OfflineVillagerNPC.WAS_INFECTED).orElse(false);
    }
    ...
    private static @NotNull LastKnownPosition lastPositionFrom(@NotNull CompoundTag tag) {
        String world = tag.getString("World").orElse("");
        ListTag list = tag.getList("Pos").orElse(new ListTag());
    ...
    private static UUID getUUIDFromTag(CompoundTag tag, String key) {
        return tag.contains(key) ? tag.getIntArray(key).map(net.minecraft.core.UUIDUtil::uuidFromIntArray).orElse(null) : null;
    }

    private static void putUUIDToTag(CompoundTag tag, String key, UUID uuid) {
        if (uuid != null) {
            tag.putIntArray(key, net.minecraft.core.UUIDUtil.uuidToIntArray(uuid));
        }
    }

    private static UUID loadUUIDFromTag(Tag tag) {
