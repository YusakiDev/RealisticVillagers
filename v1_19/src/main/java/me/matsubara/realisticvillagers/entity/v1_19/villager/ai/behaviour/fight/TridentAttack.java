...
public class TridentAttack extends Behavior<Villager> {
    ...
    private void throwTrident(VillagerNPC npc) {
        int riptide = EnchantmentHelper.getRiptide(weapon);
        if (riptide == 0) {
            ...
            weapon.hurtAndBreak(1, npc, (used) -> used.broadcastBreakEvent(hand));

            trident.tridentItem = weapon.copy();
            npc.level.playSound(null, trident, SoundEvents.TRIDENT_THROW, SoundSource.PLAYERS, 1.0f, 1.0f);
        ...
        npc.level.playSound(null, npc, sound, SoundSource.PLAYERS, 1.0f, 1.0f);
    }
