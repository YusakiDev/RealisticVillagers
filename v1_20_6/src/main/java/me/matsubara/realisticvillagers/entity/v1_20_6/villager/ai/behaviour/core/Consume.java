package me.matsubara.realisticvillagers.entity.v1_20_6.villager.ai.behaviour.core;
...
public class Consume extends Behavior<Villager> implements Exchangeable {
    ...
    private void handleEffects(VillagerNPC npc) {
        if (!shouldTriggerItemUseEffects()) return;
    ...
    private boolean shouldTriggerItemUseEffects() {
        boolean flag = food.getUseDuration() - duration > (int) ((float) food.getUseDuration() * 0.21875f);
    ...
    public static boolean isSafeFood(@NotNull Item item) {
        ...
        for (FoodProperties.PossibleEffect possibleEffect : properties.effects()) {
            if (isHarmful(possibleEffect.effect())) return false;
        }
